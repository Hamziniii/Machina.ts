...
(target, propertyKey, f) => {
    ...
    target[propertyKey] = async (...args) => {
        ...
        if (msg != null) {
            c = machina_1.Machina.subCommandMiddleware(msg === null || msg === void 0 ? void 0 : msg.content, info.separator, 1);
            _args = msg["params"] || machina_1.Machina.getArgs(c, info.separator || " ").map(machinaUtility_1.convertArgType).filter(machinaUtility_1.exists); // TODO UPDATE SUBCOMMANDS TO THIS FORMAT -> # potato.splat instead of # potato splat
            let subs, sub = null;
